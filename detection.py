...
class FaceAndNumberPlateDetection:
    def __init__(self):
        ...
        if webpage:
            ...
            self.csv_raw_path = 'model/saved_captured_data/ANPR_data/csv_raw_data/' + self.date_path + '.csv'
            self.csv_refine_path = 'model/saved_captured_data/ANPR_data/csv_refined_data/' + self.date_path + '.csv'
            self.csv_raw_face_path = 'model/saved_captured_data/captured_face_images/csv_face_raw_data/' + self.date_path + '.csv'
        else:
            # paths to save the face and plate images
            self.normal_face_image_path = 'saved_captured_data/captured_face_images/normal_images/'
            self.enhanced_face_image_path = 'saved_captured_data/captured_face_images/enhanced_images/'
            self.plate_image_path = 'saved_captured_data/ANPR_data/number_plates_images/'
            self.enhanced_plate_image_path = 'saved_captured_data/ANPR_data/enhanced_number_plates_images/'
            self.vehicle_image_path = 'saved_captured_data/ANPR_data/vehicle_images/'
            self.csv_raw_path = 'saved_captured_data/ANPR_data/csv_raw_data/' + self.date_path + '.csv'
            self.csv_refine_path = 'saved_captured_data/ANPR_data/csv_refined_data/' + self.date_path + '.csv'
            self.csv_raw_face_path = 'saved_captured_data/captured_face_images/csv_face_raw_data/' + self.date_path + '.csv'
    ...
    def start_capturing(self, path=None):
        ...
        # write all the face data in csv format
        self.functions.face_to_csv(self.face_results, self.csv_raw_face_path)

        # enhance all the saved images of face and plate at the end
        if not self.real_time_image_enhancing:
            # enhance the face images and plate
            self.esr_model.end_enhancer(self.normal_face_image_path + self.date_path + "/*",
                                        self.enhanced_face_image_path + self.date_path + '/')
            self.esr_model.end_enhancer(self.plate_image_path + self.date_path + "/*",
                                        self.enhanced_plate_image_path + self.date_path + '/')
    ...
    def detect_image(self, image):
        ...
        cv.imwrite('result.jpg', out_frame)
    ...
    def detection(self, frame, copy_frame):
        ...
        for plate_face_data in detect_face_and_plate_model.boxes.data.tolist():
            ...
            if self.plate_face_class_id[class_id] == "plate":
                ...
                ''' ENHANCE THE PLATE IMAGES '''
                self.esr_real_time(self.plate_image_path, self.enhanced_plate_image_path,
                                   self.plate_img_index, license_plate)
                ...
                ''' ENHANCE THE FACE IMAGES '''
                self.esr_real_time(self.normal_face_image_path, self.enhanced_face_image_path,
                                   self.face_img_index, face_image)
    ...
    def esr_real_time(self, img_path, enhance_path, img_idx, frame):
        # create a path to save the normal images and save it
        save_img_path = img_path + self.date_path \
                        + '/' + str(img_idx) + ".jpg"

        # save the image
        cv.imwrite(save_img_path, frame)

        if self.real_time_image_enhancing:
            # path to save the enhanced images
            save_enhanced_img_path = enhance_path + self.date_path \
                                     + '/' + str(img_idx) + ".jpg"

            # enhance the saved images with the help of ESR-GAN
            self.esr_model.real_time_enhancer(frame, save_enhanced_img_path)
...
if __name__ == '__main__':
    ...
    model.start_capturing("sample.mp4")
